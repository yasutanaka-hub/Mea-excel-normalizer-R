...
output_folder <- file.path(Sys.getenv("USERPROFILE"), "Desktop", "R", "excel_output_files")
...
for (file in file_list) {
  ...
  target_label <- tryCatch(
    read_excel(file, range = paste0("A", target_row), col_names = FALSE, col_types = "text")[[1]],
    error = function(e) NA
  )
  if (is.na(target_label) || target_label == "") target_label <- "Target"

  denom_label <- tryCatch(
    read_excel(file, range = paste0("A", denom_row), col_names = FALSE, col_types = "text")[[1]],
    error = function(e) NA
  )
  if (is.na(denom_label) || denom_label == "") denom_label <- "Denominator"
  ...
  if (is.null(tgt) || is.null(den)) next
  ...
  norm <- as_tibble(norm_mat)
  colnames(norm) <- paste0(target_label, " (per ", denom_label, ")_", sub_labels)
  ...
  # ====== 2) target 生データ ======
  tgt_out <- as_tibble(tgt_mat)
  colnames(tgt_out) <- paste0(target_label, "_", sub_labels)
  ...
  # ====== 3) denom 生データ ======
  den_out <- as_tibble(den_mat)
  colnames(den_out) <- paste0(denom_label, "_", sub_labels)
